...
##### [建议] 在函数调用、函数声明、括号表达式、属性访问、`if / for / while / switch / catch` 等语句中，`()` 和 `[]` 内紧贴括号部分不允许有空格。
...
##### [建议] 单行声明的数组与对象，如果包含元素，`{}` 和 `[]` 内紧贴括号部分不允许包含空格。
...
##### [建议] 类型检测优先使用 `Object.prototype.toString.call`。对象类型检测使用 `instanceof`。
...
3. 如需要兼顾老旧浏览器，应尽量使用数组join 拼接字符串。
...
##### [强制] `for in` 遍历对象时, 使用 `hasOwnProperty` 过滤掉原型中的属性。
...
##### [强制] 优先使用 `addEventListener / attachEvent` 绑定事件，避免直接在 HTML 属性中或 DOM 的 `expando` 属性绑定事件处理。
...
##### [强制] 使用 `addEventListener` 时第三个参数使用 `false`。
