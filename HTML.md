...
sublime Text 可以在 `view indentation` 或右下角点击出现的选框 中设置`indent Using Spaces` 以及 `indent width: 4`
...
#### [建议] `id` 建议单词全字母小写，单词间以 `-` 分隔。`class` 建议单词全字母小写，单词间以 `-` 分隔，用于JavaScript选择的class则建议用`J_class-name`格式。

解释：

统一分隔符规范的目的是避免出现多种分隔符同时出现，使得在用JavaScript进行元素选择的时候混淆不清甚至出错的情况。
...
<!-- bad -->
<div id="productDetail" class="productDetail"></div>
<!-- bad -->
<div id="product_detail" class="product_detail"></div>
```
...
#### [强制] 禁止使用有样式的 `class`作为JavaScript选择器。

解释：

不允许使用有样式定义的 class 让 JavaScript 选择某些元素，应当使用 id、J_class或属性选择的方式。
...
如果IE用户安装了Chrome Frame插件，则可以使用这个插件；如果没有安装，则使用IE浏览器最新、最好的引擎呈现，而不是以传统模式呈现。
...
3. (H5) 图片作为一个icon的存在时，使用base64编码图片。
